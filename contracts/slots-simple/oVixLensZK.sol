...
interface ISlot {
    ...
    function creationTime() external view returns (uint64);
...
contract OVixLensZK {
    ...
    function getSlotData(address _slot) public returns (SlotData memory slotData) {
        address slot = _slot;
        (, slotData.feeDenominator, slotData.creationTime, slotData.closeTime) = ISlot(slot).getFactoryData();
        address collateral = ISlot(slot).COLLATERAL();
        address debt = ISlot(slot).BORROW();
        (address cCollateral, address cDebt) = ISlot(slot).getCTokens();
        ...
        // collateral data
        slotData.collateral = collateral;
        slotData.collateralSymbol = collateralIsETH ? "ETH" : IERC20Base(collateral).symbol();
        slotData.collateralDecimals = collateralIsETH ? 18 : IERC20Base(collateral).decimals();
        slotData.cCollateral = cCollateral;

        // debt data
        slotData.cDebt = cDebt;
        slotData.debt = debt;
        slotData.debtDecimals = debtIsETH ? 18 : IERC20Base(debt).decimals();
        slotData.debtSymbol = debtIsETH ? "ETH" : IERC20Base(debt).symbol();
        ...
        (slotData.collateralSwapped, slotData.debtSwapped) = ISlot(slot).getOpenAmounts();

        // ownership & slot
        slotData.owner = ISlot(slot).OWNER();
        slotData.slot = slot;
    }
