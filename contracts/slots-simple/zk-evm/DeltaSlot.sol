...
contract DeltaSlot is AlgebraZKEVMSwapper {
    ...
    constructor(address _factory) AlgebraZKEVMSwapper() VIXTokenHolder() {
        FACTORY = _factory;
    }
    ...
    function getCTokens() external view returns (address cTokenCollateral, address cTokenBorrow) {
        ...
        address debt = BORROW;
        address collateral = COLLATERAL;
        if (debt == wrapper) return (cToken(collateral), cEther());

        if (collateral == wrapper) return (cEther(), cToken(debt));

        return (cToken(collateral), cToken(debt));
    }
