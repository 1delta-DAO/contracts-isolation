...
library AlgebraPoolAddressCalculator {
    ...
    function computeAlgebraAddress(address poolDeployer, address tokenA, address tokenB) internal pure returns (address pool) {
       if (tokenA < tokenB) 
            pool = address(
                uint160(
                    uint256(
