...
import "../../../interfaces/slot-aave/I1DeltaFactory.sol";
import "../../../interfaces/slot-aave/I1DeltaSlot.sol";
...
contract AaveSlotLens {
    ...
    function getSlotData(address _slot, address _aaveV3Pool) public view returns (SlotData memory slotData) {
        address collateral = IOneDeltaSlot(_slot).COLLATERAL();
        address debt = IOneDeltaSlot(_slot).BORROW();
        AaveUserData memory aaveData = getAaveUserDataInternal(_aaveV3Pool, _slot);
        slotData = SlotData({
            slot: _slot,
            owner: IOneDeltaSlot(_slot).OWNER(),
