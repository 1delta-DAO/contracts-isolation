...
import { AToken__factory, ERC20Mock__factory, FiatWithPermit__factory, AaveSlotLens__factory, AaveSlotFactory__factory, AaveSlot__factory, VariableDebtToken__factory } from '../types';
...
async function main() {
    ...
    const factory = await new AaveSlotFactory__factory(operator).attach(coreAddresses.factory[chainId])
    const lens = await new AaveSlotLens__factory(operator).attach(coreAddresses.lens[chainId])
    ...
    const slotContract = await new AaveSlot__factory(operator).attach(slot)
